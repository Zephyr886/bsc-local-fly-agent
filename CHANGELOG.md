...
本项目遵循语义化版本。4.2.0 按未签名、小范围测试版发布；Git tag、安装器和
...
- 运行台、果蝇管理中心和卡带游戏机新增“中文 / EN”语言切换。
- 语言选择保存在本机浏览器存储中，重启 App 后继续使用上次选择。
- 动态状态、表单提示和无障碍标签随语言切换；按钮支持键盘焦点与窄屏换行。

### 兼容性

- 仅改变本地界面资源，不改变 Profile、checkpoint、SQLite、钱包密文或卡带格式。
- 4.1.3 用户可直接覆盖安装 4.2.0；用户数据目录保持不变。

## [4.1.3] - 2026-09-22

### 修正

- Registry V4 的 `tokenURI` 现在返回 Base64 JSON，并包含固定 `image` 字段、
  `COVER_URL`、`COVER_SHA256` 和 `coverSha256` 属性。
- 独立部署台在部署后核对封面常量，并在测试铸造后回读 NFT metadata，确认图片 URL
  与哈希承诺正确后才报告成功。

### 部署边界

- 旧主网 Registry `0xade936466C62A2925635C3FC014ff45Ce8251587` 不可升级，
  其既有 NFT 不会获得图片；它应保留为无图片旧版。
- Fly Cartridge v4 文件格式和 `publish` 接口未改变。需要通过 4.1.3 独立部署台部署
  新 Registry，再由官网显式切换到新地址。

## [4.1.2] - 2026-09-22

### 修正

- Windows 安装包现在包含 V4 Python 验证器运行时所需的 Profile JSON Schema，修复
  “验证并导入文件”时的 `FileNotFoundError`。
- 同时打包 Profile 预设，确保 V3 卡带包装为 V4 的路径在安装版中可用。
- 回归测试锁定 Python 外置资源清单，并让卡带测试使用规范规定的 64 位纯十六进制
  `traitKey`。

### 兼容性

- Fly Cartridge v4 文件格式、Registry V4 合约和已部署主网地址均未改变。
- 4.1.1 用户可直接覆盖安装 4.1.2；用户数据目录保持不变。
