...
/// @notice Wallet-direct v3 trait publication with a content-hashed NFT cover.
/// @dev Neural semantics are checked by the independent off-chain verifier.
contract FlyCartridgeRegistryV3Image is ERC721, ReentrancyGuard {
    using Strings for uint256;
    // MaleCNS v1.0 sampled soma positions, CC BY 4.0. The filename embeds its SHA-256 prefix.
    ...
    constructor(uint256 expectedChainId) ERC721("Fly Cartridge V3", "FLYCT") {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        bytes memory metadata = abi.encodePacked(
            ...
            '","description":"Immutable on-chain learned fly traits. Cover: MaleCNS v1.0 sampled brain soma positions (CC BY 4.0); the cover is anatomical, not live spiking or a rendering of this token state.",',
            '"image":"', COVER_URL, '",',
