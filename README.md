...
2. 运行时先用官方 Flap Portal `getTokenV8Safe` 识别 Flap 代币，否则读取 PancakeSwap V2 池；RPC 不可用时使用由 CA 确定、可重复的合成行情，便于离线演示。
...
- K 线来自运行时最近 36 根 OHLC；PN 输入、KC 激活、APL 抑制和多巴胺读数来自当前模拟快照，并非预渲染视频或装饰图片。
