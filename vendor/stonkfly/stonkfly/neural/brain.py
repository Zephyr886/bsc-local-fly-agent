...
LIBRARY = (OUT / "physiology-v6") / (
    "libmemory.dll" if sys.platform == "win32" else
...
def build():
    ...
    temp = LIBRARY.with_name("libmemory.partial" + LIBRARY.suffix)
    if sys.platform == "win32":
        compiler = [sys.executable, "-m", "ziglang", "c++"]
        flags = ["-O3", "-std=c++17", "-shared", "-target", "x86_64-windows-gnu"]
    else:
        compiler = ["c++"]
        flags = ["-O3", "-std=c++17", "-shared", "-fPIC"]
    env = dict(os.environ)
    env["ZIG_GLOBAL_CACHE_DIR"] = str(OUT / "zig-global")
    env["ZIG_LOCAL_CACHE_DIR"] = str(OUT / "zig-local")
    subprocess.run(
        [*compiler, *flags, str(SOURCE), "-o", str(temp)],
        check=True,
        env=env,
    )
...
class MemoryBrain(NativeBrain):
    ...
    def restore(self, path):
        with np.load(path, allow_pickle=False) as a:
            ...
            # The binary hash and compiler flags are retained for audit, but
