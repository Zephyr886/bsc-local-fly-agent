        ...
        for name, info in lock.items():
            ...
            if not path.exists():
                ...
                def progress(blocks, size, total):
                    ...
                    now = time.monotonic()
                    if now - last_progress >= 5 or blocks * size >= total > 0:
                        received = min(blocks * size, total) if total > 0 else blocks * size
                        print(json.dumps({"download": name, "received_mb": round(received / 1048576, 1), "total_mb": round(total / 1048576, 1), "elapsed_seconds": round(now - started, 1)}), flush=True)
                        last_progress = now
                urllib.request.urlretrieve(info["url"], tmp, reporthook=progress)
