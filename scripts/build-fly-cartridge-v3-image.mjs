// Rebuild the wallet-direct v3 image candidate using only pinned repository inputs.
...
const name = 'contracts/FlyCartridgeRegistryV3Image.sol';
const contractName = 'FlyCartridgeRegistryV3Image';
const target = path.join(root, 'artifacts/fly-cartridge-v3-image-candidate.json');
...
if (process.argv.includes('--write')) fs.writeFileSync(target, bytes);
  ...
  'Image candidate artifact differs from pinned source or dependencies');
