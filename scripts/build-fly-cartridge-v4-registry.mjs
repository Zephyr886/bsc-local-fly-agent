// Rebuild the Registry V4 candidate using only pinned repository inputs.
...
const name = 'contracts/FlyCartridgeRegistryV4.sol';
const contractName = 'FlyCartridgeRegistryV4';
const target = path.join(root, 'artifacts/fly-cartridge-v4-registry-candidate.json');
...
if (process.argv.includes('--write')) fs.writeFileSync(target, bytes);
  ...
  'Registry V4 candidate artifact differs from pinned source or dependencies');
console.log(JSON.stringify({
  artifact: path.relative(root, target).replaceAll('\\', '/'),
