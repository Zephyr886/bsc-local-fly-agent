...
export async function recover({ rpc, address, cardId, chainId = 97 }) {
  ...
  if (contractChainId !== BigInt(chainId) || !code ||
      !same(code, expectedRuntimeCode(chainId))) {
  ...
  if (tokenId !== card.tokenId || (await client.readContract({
      address, abi: artifact.abi, functionName: 'cardIdByToken', args: [tokenId],
